...
fn opens_and_updates_record() {
    let env = Env::default();
    let contract_id = env.register(MicroInvestmentInOverseasFundsContract, ());
    let client = MicroInvestmentInOverseasFundsContractClient::new(&env, &contract_id);
    let admin = Address::generate(&env);
    let asset = Address::generate(&env);
    let owner = Address::generate(&env);

    env.mock_all_auths();
    client.initialize(&admin, &asset, &String::from_str(&env, "Micro Investment in Overseas F"));
